...
| `S/N: DX-M1-16716Y  REV.C1` | `DX-M1-16716Y`（遇空格截断） |
| `Serial: SO-BIG-2024` | `SO-BIG-2024`（`S` `O` `B` `I` 原样保留） |
| `序列号：NPU-15674X` | `NPU-15674X`（支持全角冒号） |
| `s/n: dx-m1-c4m8t6hd` | `DX-M1-C4M8T6HD`（仅转大写） |
| `S/N:QR-2025-0001` | `QR-2025-0001`（冒号后可无空格） |
| `DX-M1-H8T4Y2MD`（无标记） | **不识别** |
...
- `SN` 这类短标记会检查**单词边界**，避免匹配到其他单词内部。
- 若 OCR 将 `S/N:` 与序列号拆成两个文本框，则取下一个框的首个词元。
- 末尾的 `.` `,` `;` `:` 会被去除。

> ⚠️ **标签上没有标记就不会被识别。** 系统不会仅凭形态猜测，
> 因此请务必在序列号前印上 `S/N:`。作为交换，完全没有格式限制 ——
> **任何格式的序列号**都会被原样读出。
