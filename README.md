...
OCR 은 PP-OCRv6 이 **DEEPX DX-M1 NPU** 에서 동작합니다.
...
x86_64 와 aarch64(ARM64) 를 모두 지원합니다.
...
| 중국어 | 간체 `序列号:` `序号:` `编号:` · 번체 `序列號:` `序號:` `編號:` |

### 검증 결과

아래는 실제 **DX-M1 NPU 추론** 결과입니다. 재현하려면
`./cpp/build/serial_ocr_server --test-image <이미지>` 를 쓰세요.

**규칙 동작 확인**

| 입력 (라벨에 인쇄된 문자열) | 인식 결과 | 신뢰도 | 표기 |
|---|---|---|---|
| `SN: DX-M1-A7K3P9V2` | `DX-M1-A7K3P9V2` | 0.9913 | `SN` |
| `S/N: ABC-9981-XYZ` | `ABC-9981-XYZ` | 0.9762 | `S/N` |
| `序列号：NPU-15674X` | `NPU-15674X` | 0.9921 | `序列号` |
| `编号：NPU-15674X` | `NPU-15674X` | 0.9999 | `编号` |
...
| `DX-M1-H8T4Y2MD` (표기 없음) | — (`no_serial`) | — | — |

확인 포인트:

- `Serial: SO-BIG-2024` 가 **`SO-BIG` 그대로** 나옵니다. 이전 구현은 혼동 문자
  보정 때문에 `50-81G` 로 훼손했습니다. 이 규칙으로 바꾼 이유입니다.
- `REV.C1` 이 붙어 있어도 **공백에서 잘립니다.**
- 전각 콜론(`：`), 소문자, 콜론 뒤 공백 없음 모두 처리됩니다.
- 마지막 두 줄은 **의도된 동작**입니다. `S/N:` 뒤 첫 토큰이 `DX-M1` 이면 그대로
  `DX-M1` 이고, 표기가 없으면 추측하지 않습니다.

**동봉 라벨 시트 (`assets/serial_labels.png`) 8종**
...
측정 환경: x86_64, Ubuntu 22.04, OpenCV 4.5.4, GCC 11.4, DX-M1 (M1A).
...
| `DX-RT not found` | DX-RT 설치 후 `DXRT_INSTALLED_DIR=/path ./build.sh` |
